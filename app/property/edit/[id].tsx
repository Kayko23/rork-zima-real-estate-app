...
export default function EditPropertyScreen() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api.getProperty(String(id)),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (data && !form) {
      setForm({
        title: data.title ?? '',
        type: (data.type as 'sale' | 'rent') ?? 'rent',
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      console.log('[EditProperty] update payload', body);
      return api.updateProperty(String(id), body);
    },
    onSuccess: async () => {
      await qc.invalidateQueries({ predicate: () => true });
      Alert.alert('Succès', 'Annonce mise à jour', [{ text: 'Voir', onPress: () => router.replace({ pathname: '/property/[id]', params: { id: String(id) } }) }]);
    },
    onError: (e: unknown) => {
      console.log('[EditProperty] error', e);
      Alert.alert('Erreur', e instanceof Error ? e.message : "Impossible de mettre à jour l'annonce");
    }
  });

  const set = useCallback(<K extends keyof FormState>(k: K, v: FormState[K]) => setForm((s) => ({ ...(s as FormState), [k]: v })), []);
  ...
  if (error) {
    return (
      <View style={[styles.screen, { justifyContent: 'center', alignItems: 'center', padding: 16 }]}>
        <Text style={{ color: '#b91c1c', fontWeight: '700' }}>Erreur: {String((error as any)?.message ?? error)}</Text>
      </View>
    );
  }

  return (
    <View style={styles.screen} testID="property-edit-screen">
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 20 }} showsVerticalScrollIndicator={false}>
        <Text style={[styles.h1, { paddingTop: insets.top + 8 }]}>Modifier l'annonce</Text>
        ...
        <Pressable disabled={updateMutation.isPending} onPress={() => updateMutation.mutate()} style={[styles.cta, updateMutation.isPending && { opacity: 0.6 }]} testID="submit-edit">
          <Text style={styles.ctaTxt}>{updateMutation.isPending ? 'Mise à jour…' : 'Enregistrer les modifications'}</Text>
        </Pressable>
      </ScrollView>
    </View>
  );
