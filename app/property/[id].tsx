...
export default function PropertyDetailScreen() {
  ...
  const openMaps = () => {
    try {
      ...
      console.log("openMaps", url);
      Linking.openURL(url);
    } catch (e) {
      console.error("openMaps error", e);
    }
  };
  ...
  const openItinerary = () => {
    try {
      ...
      console.log("openItinerary", url);
      Linking.openURL(url);
    } catch (e) {
      console.error("openItinerary error", e);
    }
  };
  ...
  return (
    <View style={styles.screen} testID="property-detail-screen">
      <View style={styles.topBar}>
        <IconBtn onPress={() => router.back()} testID="btn-back">
          <ChevronLeft size={22} color="#0b3b35" />
        </IconBtn>
        <View style={{ flexDirection: "row", gap: 10 }}>
          <IconBtn onPress={() => { console.log("share pressed"); }} testID="btn-share">
            <Share2 size={20} color="#0b3b35" />
          </IconBtn>
          <IconBtn onPress={() => { console.log("favorite pressed"); }} testID="btn-fav">
      ...
      >
        <View style={{ paddingHorizontal: CARD, paddingTop: 10 }}>
          ...
          <Pressable style={styles.locationRow} onPress={openMaps} testID="btn-open-maps">
        ...
        <View style={{ paddingHorizontal: CARD, gap: 12, marginTop: 8 }}>
          ...
          <AppCard
            title="Installer ZIMA"
            subtitle="Installez l’app ZIMA pour un accès rapide et des notifications"
          />

          <Text style={styles.body}>{data.description}</Text>
          <Pressable testID="btn-read-more"><Text style={styles.linkText}>Lire la suite</Text></Pressable>
        </View>
        ...
        <View style={{ paddingHorizontal: CARD, marginTop: 18 }}>
          ...
          <AgentCard agent={data.agent} onProfile={() => { console.log("go profile"); }} onAll={() => { console.log("all listings"); }} />
        </View>
        ...
        <View style={{ paddingHorizontal: CARD, marginTop: 18 }}>
          ...
          <Pressable style={{ marginTop: 6 }} testID="btn-show-amenities">
        ...
        <View style={{ paddingHorizontal: CARD, marginTop: 18 }}>
          <View style={styles.rowBetween}>
            ...
            <Pressable testID="btn-all-reviews"><Text style={styles.linkText}>Tous les avis</Text></Pressable>
          </View>

          <ReviewCard
            name="Moussa Ba"
            date="15/11/2024"
            text="Excellent appartement, très bien situé et l’agent était très professionnel. Je recommande vivement !"
          />
        </View>
        ...
        <View style={{ paddingHorizontal: CARD, marginTop: 18 }}>
          <View style={styles.rowBetween}>
            ...
            <Pressable testID="btn-see-all-popular"><Text style={styles.linkText}>Voir tout</Text></Pressable>
          </View>
        </View>
        <FlatList
        ...
        <View style={{ paddingHorizontal: CARD, marginTop: 12, marginBottom: 24 }}>
          ...
          <View style={styles.infoSecCard}>
            ...
            <Pressable testID="btn-report">
              <Text style={[styles.linkText, { marginTop: 8 }]}>Signaler cette annonce</Text>
            </Pressable>
          </View>
        </View>
      </ScrollView>
      ...
      <StickyBar
        ...
        onCall={() => { console.log("call agent"); Linking.openURL("tel:+221700000000"); }}
        onWhatsApp={() => {
          const url = "https://wa.me/221700000000?text=Bonjour%20ZIMA";
...
function IconBtn({ children, onPress, testID }: { children: React.ReactNode; onPress: () => void; testID?: string }) {
  return (
    <Pressable onPress={onPress} style={styles.iconBtn} testID={testID}>{children}</Pressable>
  );
}
...
function Button({ label, onPress }: { label: string; onPress: () => void }) {
  return (
    <Pressable style={styles.btn} onPress={onPress} testID={`btn-${label.replace(/\s+/g, "-").toLowerCase()}`}>
...
function ButtonOutline({
  ...
  return (
    <Pressable style={styles.btnOutline} onPress={onPress} testID={`btn-outline-${label.replace(/\s+/g, "-").toLowerCase()}`}>
...
function PopularCard({ item }: { item: PopularItem }) {
  return (
    <View style={styles.popularCard}>
      ...
      <Pressable style={styles.likeBtn} testID="btn-pop-like">
...
function StickyBar({
  ...
  return (
    <View style={styles.sticky}>
      <Text style={styles.price} testID="price">
        {Intl.NumberFormat("en-US").format(price)} {currency}
      </Text>
      <View style={styles.stickyActions}>
        <Button label="Contacter" onPress={onContact} />
        <IconBtn onPress={onCall} testID="btn-call"><Phone size={18} color="#0b3b35" /></IconBtn>
        <IconBtn onPress={onWhatsApp} testID="btn-whatsapp"><ChartBar size={18} color="#0b3b35" /></IconBtn>
      </View>
    </View>
  );
}
