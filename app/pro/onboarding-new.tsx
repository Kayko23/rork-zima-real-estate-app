...
import { Step0Sector } from '@/components/pro/steps/Step0Sector';
import { Step1Identity } from '@/components/pro/steps/Step1IdentityNew';
import { Step2Contact } from '@/components/pro/steps/Step2ContactNew';
import { Step3Documents } from '@/components/pro/steps/Step3DocumentsNew';
import { Step4Profile } from '@/components/pro/steps/Step4ProfileNew';
import { Step5Review } from '@/components/pro/steps/Step5ReviewNew';

const DRAFT_KEY = '@zima.pro.kyc.draft.v2';

const STEPS = [
  { id: 0, title: "Secteur d'activité", component: Step0Sector },
...
export default function ProOnboardingScreen() {
  ...
  const methods = useForm<Partial<ProKycInput>>({
    mode: 'onChange',
    defaultValues: {
      profile: { sectors: [], categories: [], servedAreas: [], languages: [] },
      identity: { accountType: 'INDEPENDANT', idType: 'CNI' },
      contacts: {},
      documents: {},
    },
  });
  ...
  useEffect(() => {
    const subscription = methods.watch(async (values) => {
      try {
        const draft = { ...values, currentStep };
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      <View style={s.stepper}>
        {STEPS.filter(step => step.id > 0).map((step, idx) => (
      ...
      <Text style={s.stepTitle}>{STEPS[currentStep].title}</Text>
