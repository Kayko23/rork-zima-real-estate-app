...
export default function ListingForm({
  ...
  const [surface, setSurface] = useState<string>(initial?.surface ? String(initial.surface) : "");
  const [beds, setBeds] = useState<string>(initial?.beds ? String(initial.beds) : "");
  const [baths, setBaths] = useState<string>(initial?.baths ? String(initial.baths) : "");
  const [salons, setSalons] = useState<string>((initial as any)?.salons ? String((initial as any).salons) : "");
  const [desc, setDesc] = useState<string>((initial as any)?.description ?? "");
  ...
  const [category, setCategory] = useState<string>((initial as any)?.category ?? "residential");
  const [subtype, setSubtype] = useState<string>((initial as any)?.subtype ?? categories[0].types[0]);
  const [amenities, setAmenities] = useState<string[]>((initial as any)?.amenities ?? []);
  const [customAmenities, setCustomAmenities] = useState<string[]>((initial as any)?.customAmenities ?? []);
  const [customAmenityInput, setCustomAmenityInput] = useState<string>("");
  const [yearDate, setYearDate] = useState<string>((initial as any)?.yearDate ?? "");
  const [docType, setDocType] = useState<DocType | null>((initial as any)?.docType ?? null);
  const [docTypeOther, setDocTypeOther] = useState<string>((initial as any)?.docTypeOther ?? "");
  const [exactAddress, setExactAddress] = useState<string>((initial as any)?.exactAddress ?? "");
  const [orientation, setOrientation] = useState<string>((initial as any)?.orientation ?? "");
  const [landmark, setLandmark] = useState<string>((initial as any)?.landmark ?? "");
  const [phone, setPhone] = useState<string>((initial as any)?.phone ?? "");
  const [consent, setConsent] = useState<boolean>((initial as any)?.consent ?? false);
