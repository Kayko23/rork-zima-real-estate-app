...
const API = process.env.EXPO_PUBLIC_API_URL ?? "https://example.com"; // à remplacer
...
export async function uploadToBackendAsync(f: PickedLocal): Promise<{ url: string; name: string; mime: string; size?: number }> {
  ...
  return { url: json.url, name: json.name ?? f.name, mime: json.mime ?? (f.mimeType || "application/octet-stream"), size: json.size ?? f.size };
}
